...
class Application(tk.Tk):
        ...
        if self.sniffFlag is True:
            ...
            if answer is False:
                ...
                if self.startListenButton['text'] == '开始捕获':
                    ...
                    t.start()
                    print(
                        threading.current_thread().name + ' 1')  # https://blog.csdn.net/briblue/article/details/85101144
    ...
    def PDU_sniff(self):
        ...
        sniff(filter="arp or ip or ip6 or tcp or udp", prn=(lambda x: self.ip_monitor_callback(x)),
              ...
              store=0)  # iface=None 则代表所有网卡 filter="arp or ip or ip6 or tcp or udp" 可选值：ether, fddi, tr, wlan, ip, ip6, arp, rarp, decnet, tcp, udp, icmp (fddi, tr, wlan是ether的别名, 包结构很类似) https://www.cnblogs.com/cheuhxg/p/15043117.html
    ...
    def choosedEtherPDUAnalysis(self):
        ...
        choosePDUNum = self.listbox.curselection()[
            0]  # 参考代码框架560行代码获取双击鼠标左键所选中的数据报的索引，https://blog.csdn.net/m0_46489757/article/details/131054701
        choosedPacket = self.sniffDataList[choosePDUNum]  # 参考代码框架561行代码根据双击鼠标左键所选中的数据报的索引提取捕获队列里的相应数据报

        self.PDUAnalysisText.delete(1.0,
                                    END)  # 参考代码框架150行代码清空协议详细解析动态窗口，1.0表示开始位置索引，END表示结束位置索引，https://blog.csdn.net/weixin_43097301/article/details/84206539
        self.PDUAnalysisText.insert(END,
                                    'Line 1, Src: ' + 'b0:73:5d:83:84:31' + ', Dst: ' + '12:67:15:09:fa:52' + '\n')  # 在协议详细解析动态窗口输出第1行字符串，END表示结束位置索引，https://it.cha138.com/mysql/show-5911416.html
        self.PDUAnalysisText.insert(END,
                                    '   Line 2: ' + '12:67:15:09:fa:52' + '\n')  # 在协议详细解析动态窗口输出第2行字符串，END表示结束位置索引，https://it.cha138.com/mysql/show-5911416.html

        self.PDUAnalysisText.insert(END, 'str(dir(choosedPacket[Ether])): ' + str(
            dir(choosedPacket[Ether])) + '\n')  # 打印Ether数据报的所有可用字段，Ether可以为IP、ARP、TCP以及UDP

        self.PDUAnalysisText.insert(END, 'str(choosedPacket[Ether].src): ' + str(
            choosedPacket[Ether].src) + '\n')  # 调用Ether数据报的src字段，Ether可以为IP、ARP、TCP以及UDP
        self.PDUAnalysisText.insert(END, '请自己补充其余代码！' + '\n')

        self.PDUCodeText.delete(1.0,
                                END)  # 参考代码框架151行协议报文编码动态窗口，1.0表示开始位置索引，END表示结束位置索引，https://blog.csdn.net/weixin_43097301/article/details/84206539
        self.PDUCodeText.insert(END,
                                'scapy 内 hexdump 详细使用：https://blog.csdn.net/qq_20237489/article/details/81632857')  # 在协议报文编码动态窗口输出第1行字符串
    ...
    def choosedARPPDUAnalysis(self):
        ...
        self.PDUAnalysisText.delete(1.0,
                                    END)  # 参考代码框架150行代码清空协议详细解析动态窗口，1.0表示开始位置索引，END表示结束位置索引，https://blog.csdn.net/weixin_43097301/article/details/84206539
        self.PDUAnalysisText.insert(END,
                                    'Python中的时间格式的读取与转换（time模块）：' + 'https://blog.csdn.net/sinat_41482627/article/details/127643708' + '\n')  # 在协议详细解析动态窗口输出第1行字符串
        self.PDUAnalysisText.delete(1.0,
                                    END)  # 参考代码框架150行代码清空协议详细解析动态窗口，1.0表示开始位置索引，END表示结束位置索引，https://blog.csdn.net/weixin_43097301/article/details/84206539
        self.PDUAnalysisText.insert(END, '请自己补充其余代码！' + '\n')
